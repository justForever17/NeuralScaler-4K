...
export const TelemetryBar: React.FC<TelemetryBarProps> = ({
  ...
  return (
    <footer className="bg-[#12141A]/95 backdrop-blur-md border border-white/[0.08] rounded-xl px-3.5 h-12 shadow-lg flex flex-nowrap items-center justify-between gap-3 shrink-0 overflow-hidden">
      {/* Left: Hardware Telemetry Metrics */}
      <div className="flex items-center gap-3 text-xs shrink-0">
        <div className="flex items-center gap-1.5 text-gray-400 whitespace-nowrap">
          <IconCpu className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
          <span className="text-gray-300 font-medium">硬件状态:</span>
        </div>

        <div className="flex items-center gap-2.5 font-mono text-[11px] shrink-0">
          <div className="px-2 py-0.5 rounded bg-white/[0.04] border border-white/[0.06] text-gray-300 whitespace-nowrap">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button
          ...
          className={`px-5 py-1.5 rounded-lg text-xs font-semibold tracking-wide transition-all duration-200 flex items-center gap-2 shadow-md shrink-0 whitespace-nowrap ${
        ...
        >
          <IconZap className={`w-4 h-4 shrink-0 ${telemetry.isProcessing ? 'animate-bounce text-emerald-400' : ''}`} />
          <span>
            {telemetry.isProcessing 
              ? `4K 渲染中 (${percent}%)...` 
              : '⚡ 开始 4K 神经超分导出'}
